...
This is not a hypothetical requirement. Evidence review of 86 screenshots from the reference PetPooja application directly confirms this topology already exists in the product we are cloning: a "Machines" configuration panel displays a distinct **Main Server IP** and one or more **Client Machine IP** entries on the outlet's local network, and the System Config screen exposes admin tiles for "Reset Sync Code," "Database Migration," and "Remove Backup Files" — all of which only make sense if a local, outlet-resident server process owns a local database and is the thing being synced, migrated, and backed up.
