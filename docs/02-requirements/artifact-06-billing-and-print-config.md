...
- **Item wise vs. Fix per item semantics.** As noted in A.6, the precise distinction between these two container-charge modes is inferred from the field names alone and should be confirmed against actual PetPooja behavior or product intent before implementation.
...
- **`order_id_highlight_mode` full option set.** Only "Last 4 characters" is confirmed from the screenshot; the complete enumerated list of dropdown options (and whether more granular options like "Last 6 characters" exist) needs to be confirmed against the actual PetPooja source screen before the ENUM in B.3 is finalized.
