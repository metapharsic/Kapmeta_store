...
This is the consolidated, single-source decision register for Kapmeta (restaurant POS clone of KapMeta), assembled at the end of Phase 0 — Discovery. Phase 0 produced 86 validated screenshots of the reference KapMeta app (single outlet "Hotel kapila", LAN client-server topology, v126.0.1) covering 13 feature areas, which fed 9 per-screen requirement docs (artifact-01..09), a DB schema draft, an API contracts draft, a sync-architecture draft, a business-logic-rules draft, and 7 phase-execution-plan docs. During evidence review, 6 ambiguities were flagged (DEC-013..017, DEC-023) and resolved in parallel by dedicated resolution work; 6 further items (DEC-018..022, DEC-024) were logged but not re-litigated in this pass. This document folds all of that into one register.
