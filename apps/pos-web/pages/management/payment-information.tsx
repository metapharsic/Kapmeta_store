...
// Payments > Payment Information. Real fetch against
// GET /management/payment-information?from=&to=&status=&provider=&orderId=
// (apps/api/src/routes/management.ts) - queries the real payments/orders
// tables, no fabricated rows.
...
export default function PaymentInformationPage() {
  ...
  const [rows, setRows] = useState<PaymentInfoRow[] | null>(null);
  ...
  const runSearch = React.useCallback((clear = false) => {
    ...
    authedFetch(`/management/payment-information?${params.toString()}`)
      .then(async (res) => {
        if (!res.ok) {
  ...
  }, [from, to, status, provider, orderId]);
  ...
  return (
    <div className="mg-app">
      <Head>
        <title>KapMeta POS - Payment Information</title>
      </Head>
            ...
            <section className="dashboard-greeting-row">
              <div>
                <span className="breadcrumb-line">
                  <Link href="/admin">Management</Link> / Payments / Payment Information
                </span>
                <h1 className="greeting-title">Payment Information</h1>
              </div>
            </section>
              ...
              <>
                <section className="panel-card">
                  <div className="filter-row">
                    ...
                    <label className="filter-field">
                      <span>Provider</span>
                      <input className="field-input" placeholder="e.g. ZOMATO" value={provider} onChange={(e) => setProvider(e.target.value)} />
                    </label>
                    <label className="filter-field">
                      <span>Order ID</span>
                      <input className="field-input" placeholder="Order # or ID" value={orderId} onChange={(e) => setOrderId(e.target.value)} />
                    </label>
                ...
                {!loading && error && (
                  <div className="empty-state-card">
                    ...
                    <h3>Could not load payment information</h3>
                    <p>{error}</p>
                  </div>
                )}
                ...
                {!loading && !error && rows && rows.length > 0 && (
                  <section className="panel-card">
                    ...
                    <p className="footer-count">Showing {rows.length} record{rows.length === 1 ? "" : "s"}.</p>
                  </section>
                )}
              </>
            )}
