  ...
  private matchesCondition(item: any, condition: any): boolean {
    ...
    for (const [key, val] of Object.entries(condition)) {
      ...
      if (val === null || val === undefined) {
        ...
        const hasOperator = Object.keys(val).some((k) => opKeys.includes(k));

        if (hasOperator) {
          const isInsensitive = val.mode === "insensitive";
          if ("in" in val && Array.isArray(val.in)) {
            const inList = isInsensitive ? val.in.map((x: any) => String(x).toLowerCase()) : val.in;
          ...
          if ("notIn" in val && Array.isArray(val.notIn)) {
            const notInList = isInsensitive ? val.notIn.map((x: any) => String(x).toLowerCase()) : val.notIn;
            const target = isInsensitive ? String(itemVal ?? "").toLowerCase() : itemVal;
            if (notInList.includes(target)) return false;
          }
          if ("equals" in val) {
            if (isInsensitive) {
              if (String(itemVal ?? "").toLowerCase() !== String(val.equals ?? "").toLowerCase()) return false;
            } else if (itemVal !== val.equals) {
              return false;
            }
          }
          if ("not" in val) {
            if (typeof val.not === "object" && val.not !== null) {
              if (this.matchesCondition(itemVal, val.not)) return false;
            } else if (isInsensitive) {
              if (String(itemVal ?? "").toLowerCase() === String(val.not ?? "").toLowerCase()) return false;
          ...
          if ("startsWith" in val) {
            const needle = String(val.startsWith).toLowerCase();
            const haystack = String(itemVal ?? "").toLowerCase();
            if (!haystack.startsWith(needle)) return false;
          }
          if ("endsWith" in val) {
            const needle = String(val.endsWith).toLowerCase();
            const haystack = String(itemVal ?? "").toLowerCase();
  ...
  async aggregate(args?: any): Promise<any> {
    ...
    if (args?._sum) {
      for (const key of Object.keys(args._sum)) {
        result._sum[key] = list.reduce((acc, cur) => acc + (Number(cur[key]) || 0), 0);
      }
    }
    return result;
  }
