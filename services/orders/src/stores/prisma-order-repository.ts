  ...
  async getPrice(menuItemId: string, outletId: string): Promise<{ priceMinor: bigint; taxRatePercent: number } | null> {
    try {
      ...
      if (row) {
        ...
        const priceVal = rowAny.priceMinor ?? rowAny.pricePaise ?? (typeof rowAny.price === 'bigint' ? rowAny.price : BigInt(Math.round(Number(rowAny.price || 0) * (Number(rowAny.price) < 1000 ? 100 : 1))));
